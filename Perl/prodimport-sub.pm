...
#
#  'RunIdx' auf n�chsten 1000er-Bereich erh�hen:
#
...
sub  Rezeptmengen_fuer_Charge_umrechnen
{
   ...
   {  print "*** Rezeptmengen_fuer_Charge_umrechnen(): Rezeptmenge = 0 ! ***\n";
      ...
      # ---- Umrechnungsfaktor f�r die Mengen in den Rezeptschritten: ----
   ...
   {  printf ( "   Chargenmenge = %.3f (Kontrollwert)\n", $menge_charge );
      ...
      # ---- St�ckzahl der Charge: ----
   if ( $art_stueck_gewicht < 0.001 )
   {  print "*** Rezeptmengen_fuer_Charge_umrechnen(): St�ckgewicht = 0 ! ***\n";
      $stueck_charge = 1;
      #$db_ret = -1;
      #goto  ende;
   }
...
#	  }
           ...
           ($par_nr == 1) )
      {
         ...
         {  printf ( " Menge Rezept/Charge: %.3f/%.3f\n", $rs_menge, $rs_menge_neu );
            ...
            # ---- Umgerechnete Menge zur Chargenmenge hinzuf�gen (Kontrolle): ----
         $menge_charge += $rs_menge_neu;
      }

#	  # Teigzettel drucken - Sollwert bleibt unver�ndert
...
sub  Charge_und_Schritte_speichern
{
   ...
   # (@V3.3) Artikel-Linien-Nummer eingefuegt
   ( $bestell_nr, $art_linie_nr ) = @_;
   
   # (@V3.3) Wenn eine Artikel-Linien-Nummer angegeben ist - wird die Rezept-Linien-Nummer überschrieben
   if ( $art_linie_nr > 100 )
   {
         $Beh_Nr = $art_linie_nr;
   }  
   ...
   # (@V3.3) wenn eine Artikel-Linien-Nummer angegeben ist wird nur die Kopfzeile gelesen
   if ( $art_linie_nr > 100 ) { goto ende; }
...
sub  Charge_in_ArbRezepte_speichern
{
   ...
   {  print "Charge_in_ArbRezepte_speichern() ...\n";
      ...
      # ---- Floats in String umwandeln und Dezimal-Separator �ndern: ----
...
if ( $DB_NEU > 1 )
{
   ...
   (
      0,                    # ARZ_TW_Nr             Bei neuen S�tzen immer 0
      ...
      $Artikel_Einheit,     # ARZ_Art_Einheit       Artikel: St�ck oder kg, Rezept: kg
      ...
      $stueck_charge_str,   # ARZ_Sollmenge_stueck  St�ckzahl Charge
      ...
      0,                    # ARZ_geloescht         geloescht = 0: nicht gel�scht
...
sub  Schritte_in_ArbRZSchritte_speichern
{
   ...
   if ( $log_level >= 1 )
   {  print "Schritte_in_ArbRZSchritte_speichern() ...\n";
   }
...
if ( $DB_NEU > 1 )
{
   ...
   #
   #     // 19.04.2006: Fehlende Felder f�r Flat-Produktion eingebaut:
   #  ARS_KO_Nr_AlNum, ARS_KO_Bezeichnung, ARS_KO_Temp_Korr,
   #  ARS_KP_Wert,
   #  ARS_KT_Typ_Nr, ARS_KT_Rezept, ARS_KT_Bezeichnung,
   #  ARS_KT_KurzBez, ARS_KT_EinheitIndex,
...
else
{
   ...
   for ( $zeile = 0; $zeile < $rz_anz_rows; $zeile++ )
   {
      ...
      $ars_paramnr = $rz_rows[$idx];
         # ---- KT-Nr besorgen (um Textkomponente erkennen zu k�nnen): ----
      $idx = $zeile * $rz_num_fields + $idx__KT_Typ_Nr;
...
#             sonst direkt �bernehmen (z.B. bei Wassertemperatur):
...
if ( $DB_NEU > 1 )
{
      ...
      (
         0,                  # ARS_TW_Nr     Bei neuen S�tzen immer 0
         ...
         $ars_rs_par3,      # ARS_RS_Par3
            # ---- 19.04.2006: Felder f�r Flat-Produktion: ----
