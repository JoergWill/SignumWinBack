...
''' </summary>
Public Class wb_KomponParam200_Global
    ''' <summary>
    ''' Hash-Table mit den statischen Werten für die einzelnen Parameter p
    ''' </summary>
    Public Shared ktTyp200Params As New Hashtable
    ...
    Shared Sub New()
        ...
        While winback.Read
            ...
            Try
                ktTyp200Params.Add(k.ParamNr, k)
            Catch
            End Try
        End While
        winback.Close()
    ...
    Public Shared Function kt200Param(p As Integer) As wb_Global.ktTyp200Param
        If ktTyp200Params.ContainsKey(p) Then
            Return ktTyp200Params(p)
        Else
    ...
    Public Shared Function IsValidParameter(index As Integer) As Boolean
        Return ktTyp200Params.ContainsKey(index)
    End Function
End Class
