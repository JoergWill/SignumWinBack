...
''' </summary>
Public Class wb_KomponParam210_Global
    ''' <summary>
    ''' Hash-Table mit den statischen Werten für die einzelnen Parameter p
    ''' </summary>
    Public Shared ktTyp210Params As New Hashtable
    ...
    Shared Sub New()
        ...
        winback.sqlSelect(wb_Sql_Selects.setParams(wb_Sql_Selects.sqlKompTypXXX, "210"))
        ktTyp210Params.Clear()
        While winback.Read
            ...
            Try
                ktTyp210Params.Add(k.ParamNr, k)
            Catch
            End Try
        End While
        winback.Close()
    ...
    Public Shared Function kt210Param(p As Integer) As wb_Global.ktTyp200Param
        If ktTyp210Params.ContainsKey(p) Then
            Return ktTyp210Params(p)
        Else
    ...
    Public Shared Function IsValidParameter(index As Integer) As Boolean
        Return ktTyp210Params.ContainsKey(index)
    End Function
