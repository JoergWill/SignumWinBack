...
''' </summary>
Public Class wb_KomponParam220_Global
    ''' <summary>
    ''' Hash-Table mit den statischen Werten für die einzelnen Parameter p
    ''' </summary>
    Public Shared ktTyp220Params As New Hashtable
    ...
    Shared Sub New()
        ...
        winback.sqlSelect(wb_Sql_Selects.setParams(wb_Sql_Selects.sqlKompTypXXX, "=220"))
        ktTyp220Params.Clear()
        While winback.Read
            ...
            Try
                ktTyp220Params.Add(k.ParamNr, k)
            Catch
            End Try
        End While
        winback.Close()
    ...
    Public Shared Function kt220Param(p As Integer) As wb_Global.ktTyp200Param
        If ktTyp220Params.ContainsKey(p) Then
            Return ktTyp220Params(p)
        Else
    ...
    Public Shared Function IsValidParameter(index As Integer) As Boolean
        Return ktTyp220Params.ContainsKey(index)
    End Function
